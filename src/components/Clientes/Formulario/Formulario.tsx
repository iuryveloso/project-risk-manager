    ...
    return (
        ...
        `}>
            ...
            `}>
                ...
                <Entrada texto={`Nome`} tipo={`text`} valor={nome} onChange={setNome}
                    onKeyPress={props.salvarDados} cliente={new Cliente(nome, +idade, id)} />
                <Entrada texto={`Idade`} tipo={`number`} valor={idade !== 0 ? idade :''} onChange={setIdade}
                    onKeyPress={props.salvarDados} cliente={new Cliente(nome, +idade, id)} />
                <div className={`flex flex-row justify-end`}>
                    <button onClick={() => props.alternarModo()} className={`
                        flex flex-row bg-red-700 text-slate-50 px-3 py-2 
                        rounded-md hover:bg-red-800 mb-1 mr-1
                        `}>Voltar</button>
                    <button onClick={() => props.salvarDados(new Cliente(nome, +idade, id))} className={`
