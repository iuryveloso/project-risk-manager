...
export default function Entrada(props: EntradaProps) {
    return (
        <div className={`flex flex-col mb-4`}>
            ...
            <input type={props.tipo} value={props.valor} readOnly={props.SomenteLeitura}
                onChange={(valor) => props.onChange?.(valor.target.value)}
                onKeyPress={(e) => e.key === 'Enter' ? props.onKeyPress(props.cliente) : false}
                className={`
                 px-4 py-3 rounded-lg border focus:outline-none my-2
                 bg-slate-100 dark:bg-slate-600
                 border-slate-500 dark:border-slate-200
                 ${props.SomenteLeitura ? `text-slate-500 dark:text-slate-400` : `
